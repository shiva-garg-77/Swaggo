...
import ResetPassword from '../Models/LoginModels/ResetPassword.js'
...
const signupApi = async (req, res) => {
    ...
    // Instead of making an internal fetch call, directly generate tokens
...
const generate_jwt_access_token = async (account) => {
    // Look up or create profile for this user
    let profile = await Profile.findOne({ username: account.username });
    ...
    if (!profile) {
        ...
        await profile.save();
        console.log(`Auto-created profile for user: ${account.username}`);
    }
    ...
    return jwt.sign(tokenPayload, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "30m" })
}
...
const jwt_Refress_token = async (account) => {
    ...
    return jwt.sign(tokenPayload, process.env.REFRESH_TOKEN_SECRET || process.env.ACCESS_TOKEN_SECRET, { expiresIn: "7d" })
}
