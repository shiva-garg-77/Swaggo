...
# Run this script to check server health and status
...
Write-Host "📊 MongoDB connections: $($mongoConnections.Count)" -ForegroundColor $(if ($mongoConnections.Count -lt 30) { "Green" } else { "Yellow" })
...
Write-Host "• To restart server: Get-Process -Name 'node' | Stop-Process; npm start" -ForegroundColor Cyan