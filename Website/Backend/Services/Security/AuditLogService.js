...
class AuditLogService {
  ...
  async flushLogs() {
    ...
    let processedLogs = []; // Define processedLogs here so it's accessible in catch block
    
    try {
      const logsToProcess = this.logQueue.splice(0, this.batchSize);
      
      // Add integrity hashes and signatures
      processedLogs = await Promise.all(logsToProcess.map(async (log, index) => {
      ...
      this.logQueue.unshift(...processedLogs);
    } finally {
      this.isFlushing = false;
    }
  }
