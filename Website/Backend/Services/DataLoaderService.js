...
class DataLoaderService {
  ...
  async batchLoadChatsByParticipant(participantIds) {
    try {
      ...
      // Optimized query using the index on participants.profileid
      const chats = await Chat.find({ 
  ...
  async batchLoadMessagesByChat(chatIds) {
    try {
      ...
      // Optimized query using the compound index on chatid and timestamp
      const messages = await Message.find({ 
        chatid: { $in: chatIds },
        isDeleted: false 
      })
      .sort({ createdAt: -1 })
      .limit(100) // Increased limit for better performance with multiple chats
      .lean().exec();
