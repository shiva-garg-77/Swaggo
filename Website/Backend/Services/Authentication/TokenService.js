...
import SessionManagementCore from '../../Security/SessionManagement/SessionManagementCore.js';
...
class TokenService {
  ...
  constructor() {
    this.algorithms = {
      access: jwtSecurityConfig.get().algorithms.access,
      refresh: jwtSecurityConfig.get().algorithms.refresh,
      csrf: jwtSecurityConfig.get().algorithms.csrf
    };
    ...
    this.secretRotationManager = {
      ...
      gracePeriod: jwtSecurityConfig.get().security.keyRotation.gracePeriod,
      rotationInterval: jwtSecurityConfig.get().security.keyRotation.interval
    };
  ...
  async generateAccessToken(user, deviceInfo = {}, sessionContext = {}) {
    try {
      ...
      const payload = {
        // Standard claims
        sub: user.id,
        iss: jwtSecurityConfig.get().issuer,
        aud: jwtSecurityConfig.get().audience,
        iat: now,
      ...
      this.logTokenEvent('access_token_generated', {
        userId: user.id,
        tokenId,
        ipAddress: finalSessionContext.ipAddress,
        userAgent: deviceInfo.userAgent,
      ...
      const fullToken = `${this.tokenPrefixes.access}${token}`;
      console.log('🔐 TOKEN SERVICE: Full token with prefix', {
        fullTokenLength: fullToken.length,
        fullTokenPreview: fullToken.substring(0, 30) + '...'
      });
   ...
   * 🔐 SECURITY ENHANCEMENT: Supports key rotation and algorithm selection
   */
  verifyAccessToken(token) {
    try {
      const tokenWithoutPrefix = token.replace(this.tokenPrefixes.access, '');
      ...
      const verified = jwt.verify(tokenWithoutPrefix, secret, { algorithms: [algorithm] });
      return verified;
    } catch (error) {
  ...
  async revokeAccessToken(tokenId) {
    try {
      // Note: Access tokens are typically stateless JWTs, but we can revoke any stored references
      // and refresh tokens that would generate new access tokens
      await this.revokeAllUserTokens(tokenId, 'access_token_revoked');
      
      this.logTokenEvent('access_token_revoked', {
        tokenId,
      ...
      console.log(`✅ SECURITY: Access token ${tokenId} revoked (10/10 Security)`);
    } catch (revokeError) {
  ...
  /**
   * Generate secure refresh token with comprehensive claims and session fixation prevention
   * 🔐 SECURITY ENHANCEMENT: Auto-revokes old tokens for maximum security
   */
  async generateRefreshToken(user, deviceInfo = {}, sessionContext = {}) {
    try {
      const now = Math.floor(Date.now() / 1000);
      const tokenId = uuidv4();
      
      // 🔐 SECURITY ENHANCEMENT: Auto-revoke ALL old refresh tokens for this user
      // This ensures only the latest token is valid, preventing token reuse attacks
      if (this.securityConfig.autoRevokeOnNewToken && sessionContext.revokeOldTokens !== false) {
        console.log(`🔐 SECURITY: Revoking old refresh tokens for user ${user.id}`);
        
        try {
          await this.revokeAllUserTokens(user.id, 'new_refresh_token_generated');
          
          this.logTokenEvent('old_tokens_revoked_for_security', {
            userId: user.id,
            newTokenId: tokenId,
            reason: 'new_refresh_token_generated',
            timestamp: new Date(),
            ipAddress: sessionContext.ipAddress,
            security: '10/10_max_security'
          });
      ...
      const payload = {
        ...
        iat: now,
        exp: now + this.getExpirationTime('refresh'),
        jti: tokenId,
        ...
        meta: {
          ...
          strength: this.calculateTokenStrength(user, deviceInfo),
          created: now,
          algorithm: this.algorithms.refresh
        }
      };
      
      // Sign token with algorithm rotation and key ID support
      const algorithm = this.selectSigningAlgorithm('refresh', riskScore);
      const secret = jwtSecurityConfig.getSigningKey('refresh');
      const token = jwt.sign(payload, secret, { algorithm });
      
      console.log('🔐 TOKEN SERVICE: Generated refresh token', {
        userId: user.id,
        tokenId: tokenId.substring(0, 8) + '...',
        tokenLength: token.length,
        tokenPreview: token.substring(0, 30) + '...'
      ...
      // Log token generation event
      this.logTokenEvent('refresh_token_generated', {
        userId: user.id,
        tokenId,
        ipAddress: finalSessionContext.ipAddress,
        userAgent: deviceInfo.userAgent,
      ...
      const fullToken = `${this.tokenPrefixes.refresh}${token}`;
      console.log('🔐 TOKEN SERVICE: Full refresh token with prefix', {
        fullTokenLength: fullToken.length,
        fullTokenPreview: fullToken.substring(0, 30) + '...'
      });
  ...
  verifyRefreshToken(token) {
    try {
      const tokenWithoutPrefix = token.replace(this.tokenPrefixes.refresh, '');
      const decoded = jwt.decode(tokenWithoutPrefix, { complete: true });
  ...
  async revokeRefreshToken(tokenId) {
    try {
      // Note: Access tokens are typically stateless JWTs, but we can revoke any stored references
      // and refresh tokens that would generate new access tokens
      await this.revokeAllUserTokens(tokenId, 'refresh_token_revoked');
      ...
      console.log(`✅ SECURITY: Refresh token ${tokenId} revoked (10/10 Security)`);
    } catch (revokeError) {
  ...
  /**
   * Generate secure CSRF token with comprehensive claims and session fixation prevention
   * 🔐 SECURITY ENHANCEMENT: Auto-revokes old tokens for maximum security
   */
  // DEPRECATED: This method is deprecated and should not be used
  // Use the correct generateCSRFToken method instead
  async generateCSRFToken(user, deviceInfo = {}, sessionContext = {}) {
    console.warn('⚠️ DEPRECATED: Using incorrect generateCSRFToken method');
    // This was incorrectly implemented and should not be used
    // The correct implementation is the one that takes accessTokenId as a parameter
    throw new Error('Deprecated method - use the correct generateCSRFToken method');
  }
  ...
  async verifyAccessToken(token, context = {}) {
    try {
      ...
      // CRITICAL FIX: Use the DIRECT secret from .env, not the rotation manager
      // The rotation manager has a different keyId than what was used to sign the token
      const secret = process.env.ACCESS_TOKEN_SECRET || SecurityConfig.auth.jwt.accessTokenSecret;
      ...
      console.log("Now i m here after verification")
      ...
      console.log("Now i m here after security check")
  ...
  async generateRefreshToken(user, deviceInfo, sessionContext, parentToken = null) {
    try {
      ...
      await refreshTokenDoc.hashToken(tokenValue);
  ...
  async refreshTokens(refreshToken, context = {}) {
    try {
      ...
      // Handle access token format (string or object)
      let accessTokenId;
      if (typeof accessTokenResult === 'string') {
        // Extract token ID from the JWT payload for string format
        try {
          const tokenWithoutPrefix = accessTokenResult.replace(this.tokenPrefixes.access, '');
      ...
      // Generate CSRF token
      const csrfToken = await this.generateCSRFToken(user, accessTokenId, {
        ipAddress: context.ipAddress,
        userAgent: context.userAgent
      });
      ...
      if (this.securityConfig.autoRevokeOnNewToken && newRefreshTokenResult) {
        try {
          ...
          refreshTokenDoc.security.flags = [...(refreshTokenDoc.security.flags || []), 'rapid_refresh'];
      ...
      // Handle access token format for return value
      let accessTokenForReturn, riskScoreForMetadata;
      if (typeof accessTokenResult === 'string') {
        accessTokenForReturn = accessTokenResult;
        riskScoreForMetadata = 0; // String format doesn't include riskScore
      } else {
        accessTokenForReturn = accessTokenResult;
        riskScoreForMetadata = accessTokenResult.riskScore;
      }
      
      return {
        valid: true,
        accessToken: accessTokenForReturn,
        ...
        metadata: {
          ...
          riskScore: riskScoreForMetadata
        }
      };
  ...
  async generateCSRFToken(user, accessTokenId, sessionContext = {}) {
    try {
      ...
      const fullToken = this.tokenPrefixes.csrf + token;
      console.log('🛡️ Full CSRF token with prefix:', {
        fullTokenLength: fullToken.length,
        fullTokenPreview: fullToken.substring(0, 30) + '...'
      });
  ...
  initializeSecretRotation() {
    try {
      // Load current secrets - CRITICAL FIX: Use correct secret for each type!
      const secretMap = {
        access: SecurityConfig.auth.jwt.accessTokenSecret,
        refresh: SecurityConfig.auth.jwt.refreshTokenSecret,
        csrf: process.env.CSRF_SECRET || SecurityConfig.auth.jwt.accessTokenSecret
      };
      
      for (const type of ['access', 'refresh', 'csrf']) {
        const currentSecret = secretMap[type];
        
        if (!currentSecret) {
          console.error(`❌ Missing secret for type: ${type}`);
          continue;
        }
        ...
        console.log(`✅ Initialized ${type} secret (length: ${currentSecret.length})`);
      ...
      console.log('🔑 JWT secret rotation initialized for all token types');
    } catch (error) {
  ...
  async performSecurityValidation(payload, context) {
    ...
    if (payload.security?.userAgent && context.userAgent) {
      ...
      if (userAgentChanged) {
        ...
        // For very high risk changes, invalidate token (except in development)
        if (uaRiskScore > 90) {
          // DEVELOPMENT: Allow User-Agent changes on localhost for testing (e.g., mobile emulation)
          if (process.env.NODE_ENV === 'development') {
            const isDevelopmentHost = context.ipAddress && (
              context.ipAddress.startsWith('127.') ||
              context.ipAddress.startsWith('192.168.') ||
            ...
            if (isDevelopmentHost) {
              console.log('⚠️ User-Agent change in development on localhost - allowing with warning');
              console.log('   This would be blocked in production for security');
              console.log(`   Old UA: ${payload.security.userAgent.substring(0, 50)}...`);
              console.log(`   New UA: ${context.userAgent.substring(0, 50)}...`);
            } else {
              // Non-localhost in development - apply production rules
              return { 
                valid: false, 
