...
class APIGatewayService {
   ...
   */
  initialize() {
    appLogger.info('APIGatewayService initialized', {
      config: this.config,
  ...
  registerService(name, service) {
    ...
    appLogger.info('Service registered with API gateway', { name });
  }
  ...
  async routeRequest(req, res, next) {
    ...
    try {
      // Log request
      appLogger.info('API Gateway request received', {
        method,
        path,
        ip: req.ip,
      ...
      appLogger.error('API Gateway routing error', {
        error: error.message,
        stack: error.stack,
        path,
        method,
      ...
      appLogger.debug('API Gateway request completed', {
        method,
        path,
        duration,
        timestamp: new Date().toISOString()
  ...
  async handleRESTRequest(req, res, next) {
    appLogger.debug('Routing REST request', {
      path: req.path,
      method: req.method
    });
  ...
  async handleGraphQLRequest(req, res, next) {
    appLogger.debug('Routing GraphQL request', {
      path: req.path,
      method: req.method
    });
  ...
  async handleUploadRequest(req, res, next) {
    appLogger.debug('Routing upload request', {
      path: req.path,
      method: req.method
    });
  ...
  async handleHealthRequest(req, res, next) {
    appLogger.debug('Handling health check request', {
      path: req.path
    });
  ...
  async handleDefaultRequest(req, res, next) {
    appLogger.debug('Handling default request', {
      path: req.path
    });
   ...
   */
  async shutdown() {
    appLogger.info('APIGatewayService shutting down', {
