...
class CSRFProtection {
    ...
    validateCsrfToken() {
        return (req, res, next) => {
            ...
            // Skip for authentication routes that don't require existing session
            ...
            console.log(`🔍 CSRF Validation Debug - Path: ${req.path}, Method: ${req.method}`);
            
            if (exemptAuthRoutes.includes(req.path)) {
                console.log(`🔓 CSRF exemption: Allowing ${req.method} ${req.path} without CSRF token`);
                return next();
            }
            ...
            if (!token) {
                ...
                // Use direct response to avoid conflicts with JSON middleware
    ...
    getTokenEndpoint() {
        return (req, res) => {
            ...
            // Use standard JSON response to avoid conflicts with optimized JSON middleware
            res.setHeader('Content-Type', 'application/json');
