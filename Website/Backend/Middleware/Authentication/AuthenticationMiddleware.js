...
class AuthenticationMiddleware {
  ...
  async performAutoRefresh(refreshToken, authContext, res) {
    ...
    if (refreshResult.valid) {
      // Extract tokens properly - handle both string and object formats
      ...
      console.log('🔐 DEBUG: Extracted token values for cookies', {
        accessTokenType: typeof accessTokenValue,
        accessTokenPresent: !!accessTokenValue,
        accessTokenLength: accessTokenValue?.length,
        refreshTokenType: typeof refreshTokenValue,
        refreshTokenPresent: !!refreshTokenValue,
        refreshTokenLength: refreshTokenValue?.length,
        csrfTokenPresent: !!refreshResult.csrfToken,
        csrfTokenLength: refreshResult.csrfToken?.length
      });
      ...
      if (accessTokenValue && typeof accessTokenValue === 'string' && accessTokenValue.length > 0 && 
        ...
        console.error('❌ SECURITY ERROR: Cannot set cookies - tokens are invalid', {
  ...
  setAuthenticationCookies(res, tokens, additionalOptions = {}) {
    ...
    // 🔧 OPTIMIZED FIX: Enhanced sameSite with better development support
    let sameSitePolicy;
    if (isDevelopment) {
      // In development, use 'lax' for better compatibility
      // 'none' requires secure flag which might not be set in HTTP development
      sameSitePolicy = 'lax'; // Safe for HTTP development, works with localhost
      console.log('🍪 Development SameSite policy: lax (for localhost compatibility)');
    } else {
      sameSitePolicy = SecurityConfig.cookies.sameSite || 'strict'; // Production default strict
      console.log('🍪 Production SameSite policy:', sameSitePolicy);
    }
    ...
    console.log('🔐 COOKIE SET DEBUG: Token values before setting cookies', {
      ...
      csrfTokenType: typeof tokens.csrfToken,
      csrfTokenLength: tokens.csrfToken?.length
    });
    
    if (!tokens.accessToken) {
      console.error('❌ CRITICAL ERROR: Attempting to set undefined accessToken cookie');
      console.error('❌ Token object:', JSON.stringify(tokens, null, 2));
      throw new Error('Cannot set authentication cookies: accessToken is undefined');
    } else {
      // ✅ CRITICAL FIX: Validate token is a non-empty string
      if (typeof tokens.accessToken !== 'string' || tokens.accessToken.length === 0) {
        console.error('❌ CRITICAL ERROR: accessToken is not a valid string', {
          type: typeof tokens.accessToken,
          value: tokens.accessToken,
          length: tokens.accessToken?.length
        });
        throw new Error('Cannot set authentication cookies: accessToken is invalid');
      }
      
      // Access token cookie with secure prefix (shorter expiry)
      console.log(`🔐 Setting cookie: ${cookiePrefix}accessToken`);
      console.log(`🔐 Cookie value preview: ${tokens.accessToken.substring(0, 30)}...`);
      ...
      console.log(`✅ Successfully set ${cookiePrefix}accessToken cookie`);
    }
    
    if (!tokens.refreshToken) {
      console.error('❌ SECURITY ERROR: Attempting to set undefined refreshToken cookie');
      // Don't set cookie with undefined value
    } else {
      // Refresh token cookie with secure prefix (longer expiry)
      console.log(`🔐 Setting cookie: ${cookiePrefix}refreshToken with value:`, tokens.refreshToken.substring(0, 30) + '...');
    ...
    if (tokens.csrfToken) {
      console.log(`🔐 Setting cookie: ${cookiePrefix}csrfToken with value:`, tokens.csrfToken.substring(0, 30) + '...');
      ...
      console.warn('⚠️ CSRF token not provided for cookie setting');
    }
    ...
    // ✅ CRITICAL FIX: Verify cookies were actually set in response headers
    const setCookieHeaders = res.getHeader('Set-Cookie');
    console.log('🔍 VERIFICATION: Set-Cookie headers in response:', {
    ...
    if (!setCookieHeaders) {
      console.error('❌ CRITICAL ERROR: No Set-Cookie headers found in response after setting cookies!');
      console.error('❌ This indicates cookies were not properly set by res.cookie()');
    }
  }
