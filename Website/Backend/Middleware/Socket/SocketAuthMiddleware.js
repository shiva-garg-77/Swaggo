...
class SocketAuthMiddleware {
  ...
  authenticate = async (socket, next) => {
    try {
      console.log(`🔌 SOCKET AUTH: NEW CONNECTION ATTEMPT STARTED`);
      console.log(`🔌 SOCKET AUTH: Socket ID: ${socket.id}`);
      console.log(`🔌 SOCKET AUTH: Client address: ${socket.handshake.address}`);
      ...
      console.log(`🔎 SOCKET AUTH: Extracting authentication context...`);
      const authContext = this.extractSocketAuthContext(socket);
      console.log(`🔎 SOCKET AUTH: Auth context:`, {
        ipAddress: authContext.ipAddress,
        userAgent: authContext.userAgent ? 'Present' : 'Missing',
        hasAuthData: !!authContext.authData
      });
      
      // Check rate limiting for socket connections
      console.log(`🚦 SOCKET AUTH: Checking rate limiting for IP: ${authContext.ipAddress}`);
      const rateLimitResult = socketRateLimiter.isRateLimited(null, authContext.ipAddress, 'connect');
      if (rateLimitResult.limited) {
        console.log(`❌ SOCKET AUTH: Rate limit exceeded for IP: ${authContext.ipAddress}`);
      ...
      console.log(`✅ SOCKET AUTH: Rate limit check passed for IP: ${authContext.ipAddress}`);
      ...
      // Handle reconnection if session ID is provided
      const sessionId = socket.handshake.auth?.sessionId;
      if (sessionId) {
        const reconnectionResult = await WebSocketReconnectionService.handleReconnection(socket, sessionId);
        
        if (reconnectionResult.success) {
          console.log(`🔄 Socket reconnected: ${socket.id} - User: ${reconnectionResult.username}`);
          
          // Attach reconnection data to socket
          socket.isAuthenticated = true;
          socket.isAnonymous = false;
          socket.userId = reconnectionResult.userId;
          socket.username = reconnectionResult.username;
          socket.sessionId = sessionId;
          socket.isReconnection = true;
          socket.reconnectionAttempts = reconnectionResult.reconnectionAttempts;
          
          // Set up reconnected session
          this.setupAuthenticatedSession(socket, { user: { id: reconnectionResult.userId, username: reconnectionResult.username } });
          
          // Track reconnected socket
          this.authenticatedSockets.set(socket.id, {
            userId: reconnectionResult.userId,
            username: reconnectionResult.username,
          ...
          // Track user sockets
          if (!this.userSockets.has(reconnectionResult.userId)) {
            this.userSockets.set(reconnectionResult.userId, new Set());
          }
          this.userSockets.get(reconnectionResult.userId).add(socket.id);
          
          // Log successful reconnection
          this.logSecurityEvent('socket_reconnected', {
            socketId: socket.id,
            userId: reconnectionResult.userId,
            username: reconnectionResult.username,
            sessionId: sessionId,
            ipAddress: authContext.ipAddress,
            reconnectionAttempts: reconnectionResult.reconnectionAttempts
          });
          
          next();
          return;
        } else {
          console.log(`❌ Socket reconnection failed: ${socket.id} - ${reconnectionResult.reason}`);
          
          // Log failed reconnection
          this.logSecurityEvent('socket_reconnection_failed', {
            socketId: socket.id,
            sessionId: sessionId,
      ...
      console.log(`🔐 SOCKET AUTH: Starting normal authentication process...`);
      const authResult = await this.authenticateSocket(socket, authContext);
      console.log(`🔐 SOCKET AUTH: Authentication result:`, {
      ...
      if (!authResult.valid) {
        ...
        console.log(`❌ SOCKET AUTH: AUTHENTICATION FAILED - Rejecting connection: ${socket.id}`);
        console.log(`❌ SOCKET AUTH: Failure reason: ${authResult.reason}`);
        ...
        console.log(`⚠️ SOCKET AUTH: Tracking auth failure for IP: ${authContext.ipAddress}, count: ${failureCount}`);
        if (failureCount > 5) {
          console.log(`🚨 SOCKET AUTH: BRUTE FORCE DETECTED - Blocking IP: ${authContext.ipAddress}`);
      ...
      console.log(`✅ SOCKET AUTH: Socket authenticated: ${socket.id} - User: ${authResult.user.username}`);
      ...
      // Track authenticated socket with WebSocketReconnectionService
      WebSocketReconnectionService.handleNewConnection(socket, {
  ...
  async performSocketTokenRefresh(refreshToken, authContext) {
    try {
      ...
      if (refreshResult.valid) {
        ...
        const refreshTokenValue = typeof refreshResult.refreshToken === 'string'
        ...
        return {
          ...
          expiresAt: typeof refreshResult.accessToken === 'object' ? refreshResult.accessToken?.expiresAt : null,
          refreshed: true
        };
      }
  ...
  /**
   * 🔧 CRITICAL FIX #29: Enforce per-user connection limits
   */
  enforceUserConnectionLimit(userId) {
    ...
    return userConnections < this.maxSocketsPerUser;
  }
  ...
  extractSocketTokens(socket) {
    // 🔧 ALWAYS LOG for socket connections to debug cookie issues
    console.log('🔍 SOCKET AUTH: Extracting tokens from socket handshake...');
    console.log('🔍 SOCKET AUTH: Handshake details:', {
      ...
      cookiePreview: socket.handshake.headers.cookie ? socket.handshake.headers.cookie.substring(0, 200) + '...' : 'NO COOKIES',
    ...
    // 🔧 CRITICAL FIX: PRIORITIZE COOKIES FIRST (Strategy 1)
    // This ensures cookie-based authentication works even if auth object is present
    if (socket.handshake.headers.cookie) {
      console.log('🍪 SOCKET AUTH: Checking cookies for tokens (PRIORITY 1)...');
      
      const cookieString = socket.handshake.headers.cookie;
      const parsedCookies = this.parseCookies(cookieString);
      
      console.log('🍪 SOCKET AUTH: Parsed cookies:', {
      ...
      for (const tokenName of accessTokenNames) {
        if (parsedCookies[tokenName]) {
          ...
          console.log(`✅ SOCKET AUTH: Found access token in cookie: ${tokenName}`);
          console.log(`✅ SOCKET AUTH: Access token preview: ${accessToken.substring(0, 30)}...`);
          break;
        }
      }
      ...
      for (const tokenName of refreshTokenNames) {
        if (parsedCookies[tokenName]) {
          ...
          console.log(`✅ SOCKET AUTH: Found refresh token in cookie: ${tokenName}`);
          console.log(`✅ SOCKET AUTH: Refresh token preview: ${refreshToken.substring(0, 30)}...`);
          break;
        }
      }
      
      if (!accessToken) {
        console.warn('⚠️ SOCKET AUTH: No access token found in cookies!');
        console.warn('⚠️ SOCKET AUTH: Available cookies:', Object.keys(parsedCookies));
      }
    } else {
      console.warn('⚠️ SOCKET AUTH: NO COOKIE HEADER PRESENT IN SOCKET HANDSHAKE!');
    }
    
    // Strategy 2: Try auth object if no tokens from cookies
    if ((!accessToken || !refreshToken) && socket.handshake.auth) {
      console.log('🔐 SOCKET AUTH: Checking auth object for tokens (fallback)...');
      ...
      if (!accessToken) {
        ...
        if (accessToken) console.log('✅ SOCKET AUTH: Found access token in auth object');
      }
      
      if (!refreshToken) {
        refreshToken = socket.handshake.auth.refreshToken || 
                      socket.handshake.auth.refresh_token;
        if (refreshToken) console.log('✅ SOCKET AUTH: Found refresh token in auth object');
      }
    }
    
    // Strategy 3: Try query parameters if still no tokens
    if ((!accessToken || !refreshToken) && socket.handshake.query) {
      console.log('🔗 SOCKET AUTH: Checking query parameters for tokens (fallback)...');
      
      if (!accessToken) {
        accessToken = socket.handshake.query.accessToken || 
                     socket.handshake.query.access_token ||
                     socket.handshake.query.token;
        if (accessToken) console.log('✅ SOCKET AUTH: Found access token in query parameters');
      }
      
      if (!refreshToken) {
        refreshToken = socket.handshake.query.refreshToken || 
                      socket.handshake.query.refresh_token;
        if (refreshToken) console.log('✅ SOCKET AUTH: Found refresh token in query parameters');
      }
    }
