...
export default function SavedPosts({ onBack, isModal = false }) {
  ...
  const handleUnsavePost = async (postid, e) => {
  ...
  const savedPosts = data?.getUserbyUsername?.savedpost || []
        ...
        <div className="h-full bg-gray-50 dark:bg-gray-900 p-6">
          {loading ? (
            ...
            <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-6 border border-red-200 dark:border-red-800">
              <p className="text-red-600 dark:text-red-400">Error loading saved posts</p>
            </div>
          ) : savedPosts.length === 0 ? (
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <Heart className={`w-6 h-6 ${post.isLikedByUser ? 'text-red-500 fill-current' : 'text-gray-700 dark:text-gray-300'}`} />
                        </button>
                        <button 
                          onClick={(e) => {
                            e.stopPropagation()
                      ...
                      <button
                        onClick={(e) => handleUnsavePost(post.postid, e)}
                        className="hover:scale-110 transition-transform"
                        title="Unsave post"
                      >
                        <Bookmark className="w-6 h-6 text-gray-700 dark:text-gray-300 fill-current" />
                      </button>
                    </div>
