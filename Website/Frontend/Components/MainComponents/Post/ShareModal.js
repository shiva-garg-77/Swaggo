...
export default function ShareModal({ isOpen, onClose, post }) {
  ...
  // Generate share URL
  const shareUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/post/${post?.postid || post?.id || 'unknown'}`;
  const shareTitle = post?.title || 'Check out this post!';
  const shareDescription = post?.caption || post?.Description || 'Shared from our social media app';
  ...
  }, [shareUrl, shareTitle, shareDescription, onClose]);
